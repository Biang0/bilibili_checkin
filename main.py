...
def execute_coin_task(bili, user_info, config):
    try:
        ...
        logger.info(f"[北京时间 {today_cst_str()}] 今日已投币: {today_coin}/5 个 (经验值: {coin_exp})")
    ...
    try:
        ...
        want_coin = max(0, min(5, want_coin))
    except:
        want_coin = 5
    ...
    logger.info(f"📊 今日已投: {today_coin}/5 个 (北京时间 {today_cst_str()})")
    logger.info(f"🎯 计划投币: {want_coin} 个")
    ...
    for bvid in video_list:
        if attempted >= max_attempts or success >= need_coin:
            break
        attempted += 1
        ...
        if ok and "成功" in msg:
            ...
            logger.info(f"   今日累计: {today_coin}/5 个 (北京时间)")
        elif "已达上限" in msg:
    ...
    if success > 0:
        return True, f"投币完成: 成功{success}/{need_coin}个，今日累计{today_coin}/5个(北京时间)"
...
def format_push_message(all_results):
    ...
    beijing_time = datetime.now(CST).strftime('%Y-%m-%d %H:%M:%S')
    content.append(f"\n> 报告时间(北京时间): {beijing_time}")
    return "\n".join(content)
...
def main():
    ...
    all_results = []
    logger.info(f"检测到 {len(cookies)} 个账号（按北京时间执行）")

    for idx, cookie in enumerate(cookies, 1):
        logger.info(f"\n{'='*40}")
        logger.info(f"开始处理账号{idx}/{len(cookies)}（北京时间 {datetime.now(CST).strftime('%Y-%m-%d %H:%M:%S')}）")
        logger.info(f"{'='*40}")
    ...
    if all_results:
        ...
        logger.info("\n" + "="*50)
        logger.info("开始发送推送通知（北京时间）...")
        logger.info("="*50)

        send_to_pushplus(config["PUSH_PLUS_TOKEN"], "Bilibili 任务报告（北京时间）", final_msg)
